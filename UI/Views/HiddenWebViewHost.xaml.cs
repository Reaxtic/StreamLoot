        ...
        public async Task<string> CaptureProgressResponseAsync(int timeoutMs = 10000, CancellationToken ct = default)
        {
            ...
            void Handler(object? s, CoreWebView2DevToolsProtocolEventReceivedEventArgs e)
            {
                try
                {
                    ...
                    // Kick fires both ".../drops/progress" (the real, full progress payload) and
                    // ".../drops/progress/test" (an empty probe). Only the real one carries progress,
                    // so ignore the "/test" variant — otherwise its empty body wins the capture race
                    // and every campaign shows 0 progress.
                    if (!url.Contains("/api/v1/drops/progress") || url.Contains("/drops/progress/test"))
                        return;
                    ...
                    Dispatcher.InvokeAsync(async () =>
                    {
                        ...
                        try
                        {
                            ...
                            // A response with reward fields is real progress data, but an empty
                            // payload like {"data":[],"message":"Success"} is ALSO a valid response
                            // (the user simply has no progress yet) and must be accepted – otherwise
                            // the capture never completes and the caller retries forever.
                            ...
                            if (!isValidProgressPayload)
                            {
                                try
                                {
                                    JsonElement el = JsonDocument.Parse(body).RootElement;
                                    isValidProgressPayload = el.ValueKind == JsonValueKind.Object
                                        && el.TryGetProperty("data", out JsonElement dataEl)
                                        && dataEl.ValueKind == JsonValueKind.Array;
                                }
                                catch
                                {
                                    // Body isn't (yet) valid JSON – keep waiting for the real response.
                                }
                            }
